...
sentry__process_crash(const sentry_options_t *options, sentry_crash_ipc_t *ipc)
{
    ...
    sentry_path_t *consent_db_path = sentry__path_from_str(ctx->database_path);
    if (consent_db_path) {
        if (options->run) {
            sentry__run_load_user_consent(options->run, consent_db_path);
        }
        sentry__path_free(consent_db_path);
    }
